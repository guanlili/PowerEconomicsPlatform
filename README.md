...
| MySQL | `localhost:3306`，`root` / `root123`，库名 `power_economics` |
...
| 配置项 | 默认值 | 修改位置 |
|--------|--------|----------|
| 前端端口 | 8094 | `docker-compose.yml` `frontend.ports` |
| 后端端口 | 8000 | `docker-compose.yml` `backend.ports` |
| MySQL 端口 | 3306 | `docker-compose.yml` `mysql.ports` |
